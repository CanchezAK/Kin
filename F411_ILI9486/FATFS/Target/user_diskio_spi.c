...
#define SD_INI_SPEED	6 	/* скорость SPI при инициализации карты:
									...
									6 - FCLK/128,
									7 - FCLK/256    */
#define SD_MAX_SPEED	4 	/* скорость SPI после инициализации карты */
...
#define GPIO_SD_CS			SPI_CS_GPIO_Port	/* порт выбора чипа -> в CubeIDE ввести метку для соответствующего пина "SD_CS" */
#define GPIO_SD_CS_PIN		SPI_CS_Pin		/* пин порта выбора чипа */
...
static BYTE wait_ready(void)
{
	...
	do {
		res = rcvr_spi();
                  fres++;
                    HAL_Delay(10);
	} while (res != 0xFF && fres < 30); // check timeout
	return res;
}
